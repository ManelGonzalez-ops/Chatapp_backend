...
db.query("create database if not exists chatappp", err=>{
    if(err)throw(err)
...
io.on("connection", socket => {
    ...
    socket.on("JoinRoom", ({ username, room, avatarUrl }) => {
        addUser(socket.id, username, room, avatarUrl)
        socket.join(room)
    ...
    socket.on("chatMessage", (info) => {
        ...
        const fullMessage = formatMessage(info.username, info.msg, info.avatarUrl)
